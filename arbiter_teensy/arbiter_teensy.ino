...
#ifdef SER2
serialBuffer ser2 = {.serialobj = &Serial2, .STATE = 0, .recv_buf = {0}, .upstream_msg_len = 28, .type_d = "PRM", .type_u = "PRMR", .ser_count = 0};
#endif

// Sensor Serial Port
#ifdef SER3
serialBuffer ser3 = {.serialobj = &Serial3, .STATE = 0, .recv_buf = {0}, .upstream_msg_len = 1488, .type_d = "SENS", .type_u = "SNSR", .ser_count = 0};
#endif

// Strong Arm Serial Port
#ifdef SER4
serialBuffer ser4 = {.serialobj = &Serial4, .STATE = 0, .recv_buf = {0}, .upstream_msg_len = 20, .type_d = "STR", .type_u = "STRR", .ser_count = 0};
#endif

// Locomotion Serial Port
#ifdef SER5
serialBuffer ser5 = {.serialobj = &Serial5, .STATE = 0, .recv_buf = {0}, .upstream_msg_len = 20, .type_d = "loco", .type_u = "LOCR", .ser_count = 0};
#endif

// Not used
#ifdef SER6
serialBuffer ser6 = {.serialobj = &Serial6, .STATE = 0, .recv_buf = {0}, .upstream_msg_len = 1, .type_d = "", .type_u = "", .ser_count = 0};
#endif
...
#ifdef DEBUG
  Serial.println("Received data from " + String(ser->type_u) + " Device");
#endif
}
...
void process_jetson_command(serialBuffer *ser, char *type, uint32_t msg_buffer_len)
{
  ...
  char type_head[5] = "head"; // Downstream type

  if (!strcmp(type, ser->type_d) || (!strcmp(type, type_head) && ser == &ser5))
  {
...
#ifdef DEBUG
      Serial.println("No data ready: " + String(type));
#endif
      return;
    }

    if (ser == &ser3)
      Serial1.write(ser->recv_buf, ser->upstream_msg_len + IMU_DATA_LEN + R2P_HEADER_SIZE);
    else
      Serial1.write(ser->recv_buf, ser->upstream_msg_len + R2P_HEADER_SIZE);
...
void serial_irq(serialBuffer *ser)
{
  if ((*(ser->serialobj)).available())
  {
    ...
    (ser->ser_count)++;
    if (ser->ser_count < ser->upstream_msg_len)
    {
      if (ser->STATE == 0)
      {
...
{
  serial_irq(&ser2);
  if (ser2.ser_count == ser2.upstream_msg_len)
  {
...
{
  serial_irq(&ser3);
  if (ser3.ser_count == ser3.upstream_msg_len)
  {
    ...
    convert_b16_to_b8(imu_data, msg3 + ser3.upstream_msg_len, IMU_DATA_LEN / 2);
    r2p_encode(type3, msg3, ser3.upstream_msg_len + IMU_DATA_LEN, ser3.recv_buf, ser3.upstream_msg_len + IMU_DATA_LEN + R2P_HEADER_SIZE);
...
{
  serial_irq(&ser4);
  if (ser4.ser_count == ser4.upstream_msg_len)
  {
...
{
  serial_irq(&ser5);
  if (ser5.ser_count == ser5.upstream_msg_len)
  {
...
{
  serial_irq(&ser6);
  if (ser6.ser_count == ser6.upstream_msg_len)
  {
...
{
  serial_irq(&ser7);
  if (ser7.ser_count == ser7.upstream_msg_len)
  {
...
void loop()
{
  ...
  if (jetson_ser.STATE == 0xf2)
  {
    ...
    r2p_decode(jetson_ser.recv_buf, jetson_ser.ser_count, &checksum, type, msg, &upstream_msg_len);
